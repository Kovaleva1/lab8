...
public class MainController implements Initializable {
    ...
    private final double RADIUS_CIRCLE = 15;
    ...
    @FXML
    private TableColumn<MusicBand, Integer> xColumn;
    @FXML
    private TableColumn<MusicBand, Integer> yColumn;
    @FXML
    private TableColumn<MusicBand, String> creationDateColumn;
    @FXML
    private TableColumn<MusicBand, Integer> countParticipantsColumn;
    @FXML
    private TableColumn<MusicBand, Integer> singleCountColumn;
    @FXML
    private Text genreText;
    @FXML
    private TableColumn<MusicBand, MusicGenre> genreColumn;
    @FXML
    private TableColumn<MusicBand, Integer> labelColumn;
    ...
    @FXML
    private MenuItem geLanguage;
    @FXML
    private MenuItem enLanguage;
    @FXML
    private MenuItem albLanguage;
    @FXML
    ...
    private AnimationMoveCircles animationMove;
    private final boolean[] isRunning = new boolean[]{false};

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        animationMove = new AnimationMoveCircles(circles, mapPane.getPrefWidth(), mapPane.getPrefHeight());
        playImage.setFitWidth(80);
        playImage.setFitHeight(80);
        playImage.setPreserveRatio(true);
        pauseImage.setFitWidth(80);
        pauseImage.setFitHeight(80);
        ...
        moveButton.setOnAction(event -> {
            animationMove.setCircles(circles);
            if (isRunning[0]) {
                animationMove.stop();
                moveButton.setText(CurrentLanguage.getCurrentLanguage().getString("start move"));
            } else {
                animationMove.start();
                moveButton.setText(CurrentLanguage.getCurrentLanguage().getString("stop move"));
            }
    ...
    public void updateTable(Collection<MusicBand> data) {
        ...
        tableView.refresh();
        if (mapPane != null) mapPane.getChildren().removeIf(node -> node instanceof Circle);
        drawCollection();
    }
    ...
    private void drawCollection() {
        mapPane.getChildren().removeIf(node -> node instanceof Circle || node instanceof Group);
        for (MusicBand musicBand : CurrentCollectionManager.getCollectionManager().getLinkedList()) {
            ...
            Circle circle = new Circle(normalizeX(musicBand.getCoordinates().getX()),
                    normalizeY(musicBand.getCoordinates().getY()), RADIUS_CIRCLE);
            ...
            circle.setOnMouseClicked(event1 -> {
                if (mediaPlayer == null || (mediaPlayer.getStatus() != MediaPlayer.Status.PLAYING)) {
                    ...
                    mediaPlayer.setAudioSpectrumListener((double timestamp, double duration, float[] magnitudes, float[] phases) -> {
                        for (int i = 0; i < NUM_BANDS; i++) {
                            double magnitude = magnitudes[i + 1] + 60;
                            double height = magnitude * BAR_HEIGHT / 60;
                            equalizerBars[i].setHeight(height);
                            equalizerBars[i].setLayoutY(centerY - height / 2);
                        }
                    });
            ...
            circles.add(new MovingCircle(musicBand.getId(), circle, dx, dy));
            mapPane.getChildren().add(circle);
        }
    }

    private void stopAnimationMoving(){
        if (isRunning[0] && animationMove != null) {
            animationMove.stop();
            moveButton.setText(CurrentLanguage.getCurrentLanguage().getString("start move"));
            isRunning[0] = false;
            }
    ...
    private double normalizeX(Integer x) {
        ...
        return RADIUS_CIRCLE + ((x - minValue) / (maxValue - minValue)) * (mapPane.getPrefWidth() - 2 * RADIUS_CIRCLE);
    }

    private double normalizeY(Integer y) {
        double minValue = Integer.MAX_VALUE;
        double maxValue = Integer.MIN_VALUE;
        return RADIUS_CIRCLE + ((y - minValue) / (maxValue - minValue)) * (mapPane.getPrefHeight() - 2 * RADIUS_CIRCLE);
    }
    ...
    private void setLanguage() {
        ...
        moveButton.setText(currentLanguage.getString("start move"));
